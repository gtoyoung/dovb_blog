...
const TaskList = styled.div<ITaskList>`
  padding: 8px;
  background-color: ${(props) => (props.isDraggingOver ? "skyblue" : "ghostwhite")};
  flex-grow: 1;
`;
          ...
          <Container ref={provided.innerRef} {...provided.draggableProps}>
            ...
            >
              <Select
                ...
                options={ColumnType}
                value={ColumnType[ColumnType.findIndex((x) => x.value === type)]}
                isSearchable={false}
                onChange={(e) => {
                    ...
                    {tasks.length > 0 ? (
                      tasks.map((task, idx) => (
                        <div key={idx} style={{ position: "relative" }}>
                          <Task key={task.id} task={task} index={idx} parentType={type} />
                          <DelButton
                            onTouchStart={(e) => {
                              e.preventDefault();
                            }}
                      ...
                      <h5 style={{ textAlign: "center", color: "gray" }} id={"dropCaption"}>
                        Drop Here
                      </h5>
                    )}
